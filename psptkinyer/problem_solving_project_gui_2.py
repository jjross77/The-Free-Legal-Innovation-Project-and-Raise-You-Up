# -*- coding: utf-8 -*-
"""
Created on Tue Jan  2 21:09:45 2024
...
class buttons_per_quadrant():
    ...
    def add_qualitites_expand_current_qualtity_or_problem_dictionary(self,event): 
        ...
        #print(content)
        #print(f"hi: {self.problem_list_dictionary[self.problem_recorded]}")
        ...
        self.conn.commit()   
        # step 2 display results from database in the program
        # make viewable in the  current connected objects window
        #print(self.problem_list_dictionary)
        ...
        for objecttt in problem_entries:
            ...
            if qualitity_list:
                ...
                # since can only have one key with the same name will need to remember this or come up with an exception when i need to
    ...
    def switch_to_previous_galaxy(self,event):
        ...
        #print(self.current_object_or_problem)
    ...
    def switch_to_method_window(self,event):
        # so the problem is that the listboxes are not generating correctly so we are going to locate the function responsbile
        # and then modify and change the erros
        ...
        def change_names_of_textboxes():
            ...
            frames={self.frame1:15,self.frame2:12,self.frame3:13,self.frame4:10,self.frame5:5}
            for  frame, category in frames.items():
                for widget in frame.winfo_children():
                    widget.destroy()  
                if category ==13:
                    #upload_previous_methods
                    self.generate_side_lists_method_design(frame,category, title_of_text="Previous Method Steps")
                    # we might have to do this one differently liek with uploading past problems
                    continue
                if category ==5:
    ...
    def record_question_or_tool_idea_used_for_method(self,event: str)-> str:
        ...
        print('hi')
        content= re.sub('\d+:','',content) 
        # need to turn this into a list that can be appended to
    ...
    def add_step_or_modify(self,event): 
        ...
        def create_dict_or_modify():
            ...
            #print()
            self.listbox4.insert(tk.END," dog\n dog\n /n dog\n/n ")
            #self.listbox1.delete(1, tk.END)
            #self.listbox1.delete(0, tk.END)
            #content=self.text_box1.get('1.0', 'end') # textbox1 is the method window
        ...
        def reupload_method_dictionary():
            ...
            for objecttt in method_entries:
                ...
                method_step_str = temporal_placement + " "+method_step + "\n"
                self.listbox4.insert(tk.END, method_step_str) 
...
class quadrants_of_program(buttons_per_quadrant):
    ...
    def generate_side_lists_method_design(self, frame_name,category,title_of_text=""):
        ...
        if title_of_text== "Current Method Steps":
            ...
            #for i in range(10):
            #    dog='woo'
            #    self.listbox4.insert(tk.END,f'{dog}')
