# -*- coding: utf-8 -*-
"""
Created on Tue Jan 30 20:22:25 2024

@author: yyyyyyyyyyyyyyyyyyyy
"""
#1.	Fix swap for conn window. check that categories are right by testing program here # name are right
# before i can do this need to take these steps and answer these quesitons:
    #DONE  step 3 set up function to make sure pipeline before is all set up.
    #DONE# need to write funciton to delete from window more safely only specific numbers
    ##DONE will need to review the  organizing_thoughtsprogram_code to make sure i did this and ensure
    #DONE make the pipeline is tight and then trust it   
    #DONE going to trust up to labels labels_for_idea_program_2
...
# step 1 develop formal method or process to transform ideas into questions or tools
# how do i know the difference between a tool or a question?
# need to just start by trying to organize things to solve this
# or between other things?
# can the idea fall in multiple categories at the same time?
# how to i maxmize the use of an idea to make sure i use it the best and most effectively
# figure out categories or labels i want to give each idea 
#  just start transforming and then observe from example best approach
#and into other categories intot he right format program maybe
# what are the steps to get from an idea to a quesiton the pipeline or method
# what are the steps to get from an idea to a tool or objective or sub method
# how to make sure i do this correctly and fit it into one of the given categories

#DONE step 2 update newest ideas to database deal with start of pipeline and get to  labels_for_idea_program_2
# need to  make sure the ideas i create intitally on phone or comptuer are uploaded to the database
...
#problem 3.	Make it easier to access current connected steps infomration to then record these objects
# find out how to grab an object from current connected objects listbox
# paste object somewhere or set up hotkeys to grab specifci qualtites from a object or the object itself
# maybe need to reserve a listbox for this
...
#problem 5.	Make objects questions easier to quickly grab ( in different boxes)
# Turn accessing each into hotkeys the windows key 56789.
# maybe create like some sort of hotkey system that will allow me to craft methods very quickly
...
#problem 7. need to manage ideas and get everything uploaded and make it something i'm happy with
# maybe develop a app for putt ideas into the database from computer and phone

#problem 8. ACTUALLY USE THE problem solving process
...
#window.bind(starcraft_hotkey_access_elements)
def increase():
...
class buttons_per_quadrant():
    ...
    def import_tools_problems_or_existing_ideas(self, label,table_name= "public.ideas_table_3",galaxies_table=False): 
    ...
    def access_qualtities_of_object(self):
        """ """
        # in that object window want to access qualtites whether in current objects
        # or in past objects
        # hopefully accomplishing this with starcraft hotkeys
    def pre_create_galaxies(self):
        """ use defintions of words to try to pre-create galaxies of various things"""
    def fix_number_step_thing(self):
        """ fix me so i can have step numbers # that work in database """
        
    def starcraft_hotkey_access_elements(self,event):
        """quick access possible objects through clicks stuff to add it as either the problem web or  """
        
        # working with past galaxies
        ...
        def change_names_of_textboxes():
            ...
            for  frame, category in frames.items():
                for widget in frame.winfo_children():
                    widget.destroy()  
            # generate textboxes again with different titles can use same function, need to get there names
                if category ==15:
    ...
    def change_back_to_conn_window(self,event): 
         ...
         for  frame, category in frames.items():
             for widget in frame.winfo_children():
                 widget.destroy()  
             if category ==15:
                 self.generate_side_lists_method_design(frame,category, title_of_text="Possible Objects",button_color="#1d8acf")
                 continue
             if category ==18:
                 self.generate_side_lists_method_design(frame,category, title_of_text="Web-Results",button_color="#1d8acf")
                 continue
             if category == 8:
                 self.generate_side_lists_method_design(frame,category, title_of_text="Sub-Methods",button_color="#1d8acf")
                 continue
             
             if category == 4:
                 self.generate_side_lists_method_design(frame,category, title_of_text="Questions",button_color="#1d8acf")
                 continue
             else:
                 self.generate_side_lists_method_design(frame,category,button_color="#1d8acf")
                 continue
...
class quadrants_of_program(buttons_per_quadrant):
    ...
    def side_lists(self, title_of_text,label_list,frame_name, galaxies_table= False ):
    ...
    def generate_side_lists_method_design(self, frame_name,category,title_of_text="", button_color="orange"):
        ...
        if title_of_text == "Current Method Steps":
            ...
            for ii, button_type,textt in zip(range(4),button_list,text_list):
                ...
                #update_galaxies.config(bg='pink')
                update_galaxies.grid(row=3, column=ii, sticky="nsew")
                update_galaxies.bind("<Button-1>", self.handle_click)  
            return "finished"
    ...
    def qualitites_boxes(self, text_box_1_title,text_box_2_title,level=0):
        if level==0:
            ...
            self.label3 = tk.Label(master=frame,text=text_box_1_title,fg="black",bg=self.background_label_color)
            self.label3.grid(row=0,column=0)
            self.label3.config(font=('Times New Roman',10),bg='white')
            self.label4 = tk.Label(master=frame,text=text_box_2_title,fg="black",bg=self.background_label_color)
    ...
    for i in range(3):
        ...
        for j in range(0, 3):
            ...
            if i==1 and j==0:# existing galaxies to draw on  # on left side all the way down
                ...
                quad.side_lists("Possible Objects",existing_objects_in_database_list,quad.frame3,galaxies_table=True)
