...
class buttons_per_quadrant():
    ...
    def use_methods(self,event):
        ...
        # need to come up with a way to access this window directly at one point
        methods=methods_window_program()
        # step 0 where to place button in window
        # place it in sub-methods box on right
        # maybe have it open on start up of problem solving program
        # step 1 open new tkinter window
        
        # step 2 upload methods and steps
        
        #display problems
        # when double click then display the steps in a nice way
        # step 3 have option to view a specific method from a list of methods and there steps
        # step 4 click this button and view these steps
        # have option
...
class methods_window_program(quadrants_of_program):
    def __init__(self):
        ...
        self.cur = self.conn.cursor() 
        window = tk.Tk()
        window.title('Accessing Methods')
        window.columnconfigure(0, weight=1, minsize=75)
        window.rowconfigure(0, weight=1, minsize=50)
        main_method_window.main_frame1 = tk.Frame(master=window, relief=tk.RAISED, borderwidth=1)
        main_method_window.main_frame1.grid()#column=0, columnspan=4 #row=2,columnspan=2
        
        label = tk.Label(master=main_method_window.main_frame1,text="Strategies", fg="black", bg="blue")
        label.grid(row=0,column=0)
        label.config(font=('Times New Roman',10),bg='white')
        self.listboxxy1 = tk.Listbox(main_method_window.main_frame1,width=100) 
        self.listboxxy1.grid(row=1,column=0)#row=1,column=0, columnspan=3
        self.listboxxy1.bind('1', self.show_method_steps)
        self.cur.execute(f"""SELECT * FROM methods_table ;""")
        methods_worked_on= self.cur.fetchall()
        ...
        for problemmmm in self.list_of_problems:
            self.listboxxy1.insert(tk.END, problemmmm)  
        
        
        main_method_window.main_frame2 = tk.Frame(master=window, relief=tk.RAISED, borderwidth=1)
        main_method_window.main_frame2.grid(row=0,column=1)#column=0, columnspan=4 #row=2,columnspan=2
        
        label = tk.Label(master=main_method_window.main_frame2,text="Actions", fg="black", bg="blue")
        label.grid(row=0)
        label.config(font=('Times New Roman',10),bg='white')
        self.listboxxy2 = tk.Listbox(main_method_window.main_frame2,width=100) 
        self.listboxxy2.grid(row=1)#row=1,column=0, columnspan=3
        self.listboxxy2.bind('1', self.show_problemss)
        
        main_method_window.main_frame2 = tk.Frame(master=window, relief=tk.RAISED, borderwidth=1)
        main_method_window.main_frame2.grid(row=0,column=2)#column=0, columnspan=4 #row=2,columnspan=2
        
        label = tk.Label(master=main_method_window.main_frame2,text="Effects", fg="black", bg="blue")
        label.grid(row=0)
        label.config(font=('Times New Roman',10),bg='white')
        self.listboxxy3 = tk.Listbox(main_method_window.main_frame2,width=100) 
        self.listboxxy3.grid(row=1)#row=1,column=0, columnspan=3
    ...
    def show_method_steps(self,event):
        ...
        if method_step_name_dic:
            for methoddd in empty_method_list:
                ...
                insert_value1=f"{methoddd[0]}: {methoddd[1]}"
                insert_value1=re.sub("[\[\]{}\']","",insert_value1)
                self.listboxxy1.insert(tk.END, insert_value1) 
        if action_step_name_dic:
            ...
            for actionn in empty_action_list:
                ...
                insert_value2=f"{actionn[0]}: {actionn[1]}"
                insert_value2=re.sub("[\[\]{}\']","",insert_value2)
                self.listboxxy2.insert(tk.END, insert_value2)    
