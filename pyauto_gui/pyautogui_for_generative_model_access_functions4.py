...
process file with re from ctrl p: "."
...
"""
    ...
    def add_function_str_from_sql_to_file(self,search_prompt,column_list,strr_for_py_auto_script):
        ...
        strr_for_py_auto_script+=f"\nelse:"
        strr_for_py_auto_script+=f"\ncontinue"
        ###grab_text_from_clipboard_re_search_and_add_text_to_clipboard
        strr_for_py_auto_script+=f"\ndef grab_text_from_clipboard_re_search_and_add_text_to_clipboard(re_pattern,groupp=0):"
        strr_for_py_auto_script+=f"\nimport pyperclip"
        strr_for_py_auto_script+=f"\nimport re"
        strr_for_py_auto_script+=f"\ntext_to_process=pyperclip.paste()"
        strr_for_py_auto_script+=f"\nsearch_result=re.search(re_pattern,text_to_process)"
        strr_for_py_auto_script+=f"\nif search_result:"
        strr_for_py_auto_script+=f"\nsearch_result=search_result.group(groupp)" # change if special pattern
        ...
        strr_for_py_auto_script+=f"\ncounterr+=1"
        strr_for_py_auto_script+=f"\nif counterr>30:"
        strr_for_py_auto_script+=f"\nrestart_master_script(input_values) # add the script name self.script_name that this function is being called in here to restart it recursing on the script when something is not working"
        strr_for_py_auto_script+=f"\nreturn"
        strr_for_py_auto_script+=f"\ntemplate_present_value,h, w=self.check_for_template(template_path)"
        strr_for_py_auto_script+=f"\nif template_present_value[0].any():"
        strr_for_py_auto_script+=f"\npyautogui.click(int(template_present_value[1][0]+w/2),int(template_present_value[0][0]+h/2))"
        ...
        strr_for_py_auto_script+=f"\nexcept:"
        strr_for_py_auto_script+=f"\ninput('depending on what you type can move in the script')"
...
class pyautogui_for_generative_model_access_functions_child(pyautogui_for_generative_model_access_functions):
    ...
    def on_mouse_scroll(self,x, y, dx, dy):
        ...
        print(f" {'Scrolled {0} at {1}'.format('down' if dy < 0 else 'up',(x, y))}")
