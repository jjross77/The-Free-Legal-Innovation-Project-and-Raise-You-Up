...
"""
    ...
    def screen_record(self,screen_shot_time):
        ...
        active_window_name=re.sub(r"\"","",active_window_name)
        active_window_name=re.sub(r",","",active_window_name)
        if "C:\Windows\system32\cmd.exe - C:\ProgramData\Anaconda3\python.exe" in active_window_name:
            len_of_str=len("C:\Windows\system32\cmd.exe - C:\ProgramData\Anaconda3\python.exe")
            active_window_name=active_window_name[len_of_str:].strip()
            #active_window_name=re.sub(r"C:\Windows\system32\cmd.exe - C:\ProgramData\Anaconda3\python.exe","",active_window_name)
            print('MEOWWW')
            print(active_window_name)
            #C:\Windows\system32\cmd.exe - C:\ProgramData\Anaconda3\python.exe  C:\Users\yyyyyyyyyyyyyyyyyyyy\Documents\Coding\problem
    ...
    def get_active_url_name_from_browsers_history(self):
        ...
        #print("\nFirefox History:")
        lowered_active_window_name=self.active_window_name
        ...
        if firefox_history:
            ...
            highest_counted_website=re.sub(r"\'","",str(highest_counted_website))
            highest_counted_website=re.sub(r"\"","",highest_counted_website)
            highest_counted_website=re.sub(r",","",highest_counted_website)            
  ...
  saved_screen_shots_text_list=[]"""
    ...
    def create_template_mouse_click(self,keyy,timee,path_to_frame,folder_name,box_size=20):
        ...
        print(path_to_frame)
        mouse_cordinate_split=re.search(r"(\d+)`(\d+)",keyy)
    ...
    def add_mouse_click(self,strr_for_py_auto_script,keyy,keyboard_or_mouse,timee,path_to_frame,active_window_name,folder_name,remote=False):
        ...
        if keyboard_or_mouse=="mouse_press_l" or keyboard_or_mouse=="mouse_press_r" :
            if remote==True:
                mouse_cordinate_split=re.search(r"(\d+)`(\d+)",keyy) 
                #mouse_cordinate_split=re.search(r"\d+`\d+",top_left_mouse_click)
                x=int(mouse_cordinate_split.group(1))
                y=int(mouse_cordinate_split.group(2))
  ...
  """
    ...
    def create_pyauto_gui_function_str(self,pyautogui_page_action_dic,previous_active_window_url,previous_active_window,code_file_name_only,code_file_name_path_linux,add_additional_functions=["check_for_template(self"],remote=False):
        ...
        for i,keyboard_or_mouse in enumerate(pyautogui_page_action_dic["keyboard_or_mouse_list"]):          
            press_scroll_move=pyautogui_page_action_dic["press_scroll_move_list"][i]
            timee=pyautogui_page_action_dic["timee_list"][i]
            active_window_name=pyautogui_page_action_dic["active_window_name_list"][i]
            path_to_frame=pyautogui_page_action_dic["path_to_frame_list"][i]
            spacing_to_add=pyautogui_page_action_dic["spacing_to_add_list"][i]
            key_value=pyautogui_page_action_dic["key_value_list"][i]
            active_window_url=pyautogui_page_action_dic["active_window_url_list"][i]
            ...
            ### keyboard stuff
            if key_value in keys_to_look_for:
                strr_for_py_auto_script+=f"\n  print('{key_value}: {i}')"
                strr_for_py_auto_script+=dictionary_of_keyys[key_value]
                self.previous_keyboard_or_mouse=keyboard_or_mouse      
                continue

            if keyboard_or_mouse=="keyboard" or "keyboard_typing":
                if key_value=="@@@":
                    continue
                strr_for_py_auto_script+=f"\n  print('{key_value}: {i}')"
                strr_for_py_auto_script+=f"\n  pyautogui.write({key_value})" 
                #print(f"\n  pyautogui.write({key_value})" )
                self.previous_keyboard_or_mouse=keyboard_or_mouse 
                continue
            ### MOUSE STUFF
            ...
            if keyboard_or_mouse=="mouse_press_l" or keyboard_or_mouse=="mouse_press_r":
                ...
                spacing_to_add=spacing_to_add
                strr_for_py_auto_script+=spacing_to_add               
                if keyboard_or_mouse=="mouse_press_l":
                    strr_for_py_auto_script+=f"\n  print('{key_value}: {i}')"
                    if remote==True:                        
                        strr_for_py_auto_script= self.add_mouse_click(strr_for_py_auto_script,press_scroll_move,keyboard_or_mouse,timee,path_to_frame,active_window_name,self.dir_to_store_script,remote=True)
                    if remote==False:
                        strr_for_py_auto_script= self.add_mouse_click(strr_for_py_auto_script,press_scroll_move,keyboard_or_mouse,timee,path_to_frame,active_window_name,self.dir_to_store_script)      
                self.previous_keyboard_or_mouse=keyboard_or_mouse 
                continue        
            if keyboard_or_mouse=="mouse_move":
                self.move_to_list_counterr+=1
                stored_mouse_move_list_items=press_scroll_move
                strr_for_py_auto_script+=f"""
  ...
  self.move_mousey(move_to_list{self.move_to_list_counterr})"""
            ...
            if keyboard_or_mouse=="mouse_scroll":
                amount_to_scroll=press_scroll_move
                strr_for_py_auto_script+=f"\n  pyautogui.scroll({amount_to_scroll})"
                continue
...
class pyautogui_for_generative_model_access_functions_child(pyautogui_for_generative_model_access_functions):
    ...
    def on_mouse_scroll(self,x, y, dx, dy):
        """ start out with empty list in each column
        use delimiters to ensure keep correct ordering of values assocaited
        add a speciic information from each listner stored as single dicitonary
        end string [,@@@,@@@]
        need to setup to retrieve values in each column from sql on big black
        from thsi dicitonary we are creating here
        
        # if first value will need to just do @@@      
        # to deal with move_list on reupload add delimiters as well
        # instead of blanks use @@@ to keep order
        re.split(",")
        # just control to make sure no commas are put into the lists we upload
        # otherwidse come up with a special solution
        # need to remove commas from active window name to make this work
        # remove commas from path_to_frame_list
        # remove commas from spacing_to_add_list
        # just remove commas from all of these and then can 
        re.split(",",value_str[1:-1])
        [mouse_scroll,mouse_scroll,mouse_scroll]
        [Scrolled {0} at {1}'.for,Scrolled {0} at {1}'.for,Scrolled {0} at {1}'.for]
         [self.previous_active_window,self.previous_active_window]
         [path_to_frame_list,path_to_frame_list]
         [spacing_to_add_list,spacing_to_add_list,spacing_to_add_list]
         [key_value_list,key_value_list,key_value_list]
         [active_window_url_list,active_window_url_list]
         split mouse_move super important
         [***{x}`{y}***{x}`{y}***{x}`{y},***{x}`{y}***{x}`{y}***{x}`{y}]
         (1) re.split(",",value_str[1:-1])# assocaited with a single action
         for move_values in move_values_list:# deal with the interior lists for that page
             (2)=re.split("***",move_values)  # to get single movement          
             (3)[x,y]=re.split("`",move_values)# to get xy
        ...
        if self.last_input=="mouse_scroll":
            ...
            self.pyautogui_page_action_dic["press_scroll_move_list"][-1]+=amount_to_scroll
            
            # will need to manage this
            return
        else:
            self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("mouse_scroll")
            self.pyautogui_page_action_dic["press_scroll_move_list"].append(amount_to_scroll)
            self.pyautogui_page_action_dic["timee_list"].append(timee)
            self.pyautogui_page_action_dic["active_window_name_list"].append(self.previous_active_window)
            self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
            self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
            self.pyautogui_page_action_dic["key_value_list"].append("@@@")
            self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
    ...
    def on_mouse_move(self, x, y):
        ...
        if self.last_input!="mouse_move":
            ...
            # how do i deal with stored mouse move list
            # split [[],]
            self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("mouse_move")
            self.pyautogui_page_action_dic["press_scroll_move_list"].append(self.stored_mouse_move_list)
            self.pyautogui_page_action_dic["timee_list"].append(timee)
            self.pyautogui_page_action_dic["active_window_name_list"].append(self.previous_active_window)
            self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
            self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
            ...
            self.stored_mouse_move_list.append(f"***{x}`{y}")
            self.last_input="mouse_move"
    ...
    def mouse_pynput_on_click(self,x, y, button, pressed):
        ...
        if self.previous_active_window!="" and self.active_window_name!=self.previous_active_window:  
            if self.start_recording==False:
                ...
                self.create_pyautogui_file_of_inputs_and_video_2(self.pyautogui_page_action_dic,previous_active_window_2,previous_active_window_url)
                print('new upload!')
                print(self.pyautogui_page_action_dic)
                self.pyautogui_page_action_dic={
                    # only upload these columns for doing processing sql and doing work on big black to generate new pyautogui scripts
                    "keyboard_or_mouse_list":[],   
                    "press_scroll_move_list":[],
                    "timee_list":[],
                    "active_window_name_list":[],
                    "path_to_frame_list":[],
                    "spacing_to_add_list":[],
                    "key_value_list":[],
                    "active_window_url_list":[],
                    # just dont upload these and then dont need to worry aobut redoing processing sql function
                     "script_name":"",
                     "computer_name":"",      
                     "other_associated_files":""}
        ...
        if button == mouse.Button.right:# need to add in right mosue clicks
            ...
            if self.last_input=="mouse_press_r":
                self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("mouse_release_r")
                self.pyautogui_page_action_dic["press_scroll_move_list"].append(f"{x}`{y}")
                self.pyautogui_page_action_dic["timee_list"].append(query_time)
                self.pyautogui_page_action_dic["active_window_name_list"].append(self.active_window_name)
                self.pyautogui_page_action_dic["path_to_frame_list"].append(path_to_frame)#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append(spacing_to_add)
                self.pyautogui_page_action_dic["key_value_list"].append("@@@")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)            
                ...
                self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("mouse_press_r")
                self.pyautogui_page_action_dic["press_scroll_move_list"].append(f"{x}`{y}")
                self.pyautogui_page_action_dic["timee_list"].append(query_time)
                self.pyautogui_page_action_dic["active_window_name_list"].append(self.active_window_name)
                self.pyautogui_page_action_dic["path_to_frame_list"].append(path_to_frame)#delimiter to retreive list
        ...
        if button == mouse.Button.left:
            if self.last_input=="mouse_press_l":
                self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("mouse_release_l")
                self.pyautogui_page_action_dic["press_scroll_move_list"].append(f"{x}`{y}")
                self.pyautogui_page_action_dic["timee_list"].append(query_time)
                self.pyautogui_page_action_dic["active_window_name_list"].append(self.active_window_name)
                self.pyautogui_page_action_dic["path_to_frame_list"].append(path_to_frame)#delimiter to retreive list
                ...
                self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("mouse_press_l")
                self.pyautogui_page_action_dic["press_scroll_move_list"].append(f"{x}`{y}")
                self.pyautogui_page_action_dic["timee_list"].append(query_time)
                self.pyautogui_page_action_dic["active_window_name_list"].append(self.active_window_name)
                self.pyautogui_page_action_dic["path_to_frame_list"].append(path_to_frame)#delimiter to retreive list
    ...
    def keyboard_on_press_pynput(self,event):
        ...
        self.last_input="keyboard"# this is for the mouse do know when to add vlaues to the list
        try:
            pressed_button='{0}'.format(event.char)
            print(pressed_button)
            if len(self.pyautogui_page_action_dic["keyboard_or_mouse_list"])==0:
                last_keyboard_or_mouse_press="blank"
            else:
                last_keyboard_or_mouse_press=self.pyautogui_page_action_dic["keyboard_or_mouse_list"][-1]
            ...
            if pressed_button=="`":
                if self.start_recording==True:
                    ...
                    previous_active_window_url=self.active_window_url
                    previous_active_window_2=previous_active_window_2[:40]
                    self.create_pyautogui_file_of_inputs_and_video_2(self.pyautogui_page_action_dic,previous_active_window_url)
                    self.pyautogui_page_action_dic={
                        # only upload these columns for doing processing sql and doing work on big black to generate new pyautogui scripts
                        "keyboard_or_mouse_list":[],   
                        "press_scroll_move_list":[],
            ...
            if pressed_button=="":
                ...
                self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("keyboard")
                self.pyautogui_page_action_dic["press_scroll_move_list"].append(f"@@@")
                self.pyautogui_page_action_dic["timee_list"].append(timee)
                self.pyautogui_page_action_dic["active_window_name_list"].append(self.active_window_name)
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("ctrl_c")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)  
            ...
            if pressed_button=="":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("ctrl_a")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if pressed_button=="":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("ctrl_p")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if pressed_button==",":# we do this to avoid splititng errors downstream             
                self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("keyboard")
                self.pyautogui_page_action_dic["press_scroll_move_list"].append(f"@@@")
                self.pyautogui_page_action_dic["timee_list"].append(timee)
                self.pyautogui_page_action_dic["active_window_name_list"].append(self.active_window_name)
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("comma")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
                #self.keyboard_and_mouse_press_and_time_list.append(["keyboard","ctrl_p",time.time()]) 
                print('comma')
                # add a process file thing here? and re maybe? add in optional
            ...
            if pressed_button=="":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("ctrl_s")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            last_value=self.pyautogui_page_action_dic["key_value_list"][-1]
            ...
            if last_keyboard_or_mouse_press=="keyboard" and self.ctrl_activated==False:# last press was keyboard and not control
                if last_value in self.ctrl_command_list:# to stop a write command
                    ...
                    self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                    self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                    self.pyautogui_page_action_dic["key_value_list"].append('{0}'.format(event.char))
                    self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
                    return
            ...
            if last_keyboard_or_mouse_press=="keyboard_typing": # last press was mouse
                # add to the string
                self.pyautogui_page_action_dic["key_value_list"][self.current_keyboard_text_placement_in_dic]+='{0}'.format(event.char)
                return
            
            else:
                # fixing keyboard              
                self.pyautogui_page_action_dic["keyboard_or_mouse_list"].append("keyboard_typing")
                self.pyautogui_page_action_dic["press_scroll_move_list"].append(f"@@@")
                self.pyautogui_page_action_dic["timee_list"].append(timee)
                self.pyautogui_page_action_dic["active_window_name_list"].append(self.active_window_name)
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                ...
                # use this to update this reference value in the pyautogui page action dic until we swap to using the mouse again
                self.current_keyboard_text_placement_in_dic=len(self.pyautogui_page_action_dic["active_window_url_list"])-1
                self.keyboard_marker=True
                #self.keyboard_and_mouse_press_and_time_list.append(["keyboard",'{0}'.format(event.char),time.time()]) 
            ...
            #list_len=len(self.pyautogui_page_action_dic["keyboard_or_mouse_list"])
            #if list_len>0:
            if len(self.pyautogui_page_action_dic["keyboard_or_mouse_list"])==0:
                last_keyboard_or_mouse_press="blank"
            else:
            ...
            if str('{0}'.format(event))=="Key.space" and last_keyboard_or_mouse_press=="keyboard_typing":
                self.pyautogui_page_action_dic["key_value_list"][self.current_keyboard_text_placement_in_dic]+=" "
                print('enter!')
                return
            ...
            if str('{0}'.format(event))=="Key.f12":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("F12")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if str('{0}'.format(event))=="Key.down":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("DOWN")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if str('{0}'.format(event))=="Key.up":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("UP")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if str('{0}'.format(event))=="Key.left":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("LEFT")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if str('{0}'.format(event))=="Key.right":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("RIGHT")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if str('{0}'.format(event))=="Key.tab": 
                 if self.tab_skip== True:
                     ...
                     return
                        ...
                        self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                        self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                        self.pyautogui_page_action_dic["key_value_list"].append("tab")
                        self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if str('{0}'.format(event))=="Key.alt_l":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("alt")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
            ...
            if str('{0}'.format(event))=="Key.shift":
                ...
                self.pyautogui_page_action_dic["path_to_frame_list"].append("@@@")#delimiter to retreive list
                self.pyautogui_page_action_dic["spacing_to_add_list"].append("@@@")
                self.pyautogui_page_action_dic["key_value_list"].append("shift")
                self.pyautogui_page_action_dic["active_window_url_list"].append(self.active_window_url)
...
navigator.clipboard.writeText(textToCopy)
  ...
  .catch((err) => {
    ...
    def add_or_create_pyautogui_function_exe_files(self,exe_and_main_file_path,code_file_name_no_path,multiprocessing=False, remote=False):
        """ """
        import re
        import os
        import time
...
xdotool mousemove 100 200 
    ...
    def insert_into_postgres_local(self,column_name_data_type_dic,table_name=""):
     '''         # maybe modify up stream so we can use comma as a delimiter
     yes going to do that
     need to remove commas/control from active window name and active window url
     and if fix active window name this will automcially fix script name'''
     import psycopg2
     import re
     self.conn = psycopg2.connect(dbname='can_law_accessible', user='postgres', password='Meganiscute')
     self.cur = self.conn.cursor()
     ...
     for i, (table_column, column_value) in enumerate(column_name_data_type_dic.items()):
         ...
         column_value=column_value.replace("\"","")
         #column_value=column_value.replace(","," ")
         # going to keep comma delimiter for retrieving data for later
         column_value=column_value.replace("'","")
         column_value=re.sub("\"","",column_value)
         if i ==0:
             table_columns =  f"{table_column}"
    ...
    def generate_local_pyautogui_scripts(self,pyautogui_page_action_dic,previous_active_window_url,previous_active_window):
        """ """
        import time
        import os
        import re
        ...
        pyauto_file_dir_list=os.listdir(root_folder)
        ...
        print(f"previous_active_window {previous_active_window}")

        #print(f"pyauto_file_dir_list{pyauto_file_dir_list}")
        ...
        remote=False
        python_file_str=self.create_pyauto_gui_function_str(pyautogui_page_action_dic,previous_active_window_url,previous_active_window,code_file_name_only,code_file_name_path_linux,remote=remote)
        ...
        single_process_exe_file_name=self.add_or_create_pyautogui_function_exe_files(self.dir_to_store_script,code_file_name_no_path_for_function_import,multiprocessing=False)
        multi_process_exe_file_name=self.add_or_create_pyautogui_function_exe_files(self.dir_to_store_script,code_file_name_no_path_for_function_import,multiprocessing=True)
        ...
        relevent_local_file_names=[code_file_name_path,single_process_exe_file_name,multi_process_exe_file_name]
        return relevent_local_file_names
...
for i in xrange(n):
  ...
  p.wait()"""     
    ...
    def create_pyautogui_file_of_inputs_and_video_2(self,pyautogui_page_action_dic,previous_active_window,previous_active_window_url):
        ...
        restructurung the keyboard_and_mouse_press_and_time_list
        """ 
        ###
        import time
        timee=time.time()
        relevent_local_file_names=self.generate_local_pyautogui_scripts(self.pyautogui_page_action_dic,previous_active_window_url,previous_active_window)
        ...
        column_name_data_type_dic={ 
         # only upload these columns for doing processing sql and doing work on big black to generate new pyautogui scripts
         "keyboard_or_mouse_list":pyautogui_page_action_dic["keyboard_or_mouse_list"],   
         "press_scroll_move_list":pyautogui_page_action_dic["press_scroll_move_list"],
         "timee_list":pyautogui_page_action_dic["timee_list"],
         "active_window_name_list":pyautogui_page_action_dic["active_window_name_list"],
         "path_to_frame_list":pyautogui_page_action_dic["path_to_frame_list"],
         "spacing_to_add_list":pyautogui_page_action_dic["spacing_to_add_list"],
         "key_value_list":pyautogui_page_action_dic["key_value_list"],
         "active_window_url_list":pyautogui_page_action_dic["active_window_url_list"],
         # just dont upload these and then dont need to worry aobut redoing processing sql function
