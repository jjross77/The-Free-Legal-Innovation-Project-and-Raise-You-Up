...
"""
  ...
  saved_screen_shots_text_list=[]"""
   ...
   self.wait_till_template_present(r'{self.template_location_str_linux}') """
    ...
    def create_new_autogui_file(self,strr_for_py_auto_script,previous_active_window_url):
        """ """
  ...
  os.environ['DISPLAY'] = ':0'"""
    ...
    def create_pyauto_gui_function_str(self,keyboard_and_mouse_press_and_time_list,previous_active_window_url,add_additional_functions=["check_for_template(self"]):
        ...
        strr_for_py_auto_script=self.create_new_autogui_file(strr_for_py_auto_script,previous_active_window_url)  
...
class pyautogui_for_generative_model_access_functions_child(pyautogui_for_generative_model_access_functions):
    ...
    def mouse_pynput_on_click(self,x, y, button, pressed):
        ...
        previous_active_window=self.active_window_name
        ...
        # if tab between tabs can screw this up
        # this is to decide to create a file or not
        if previous_active_window!="" and self.active_window_name!=previous_active_window:  
            for screen_namee in self.screens_where_screen_shots_wanted_list:
                if screen_namee in previous_active_window:
                    ...
                    break
        # this generates the pyautogui code
        for screen_namee in self.screens_where_screen_shots_wanted_list:
            if screen_namee in self.active_window_name:
            ...
            if screen_namee in self.active_window_name:
                ...
                try:
                    ...
                    if self.keyboard_and_mouse_press_and_time_list:
                        ...
                        # new buttons to write
                        #DONE FIX ALT TAB to return to previous web page
                        # alt tab adds a alt tab
                        # just fix alt tab for this one to work
                        # or just make alt tab work to return the page
                        # lets do that instead
                        # thiss should wokr lets test it
                         
                        
                        # DONE if screenshot unique FIX THIS
                        # if we make screenshot uunique enough we dont need
                        # to do this because templates wont be the same
                        # so dont need to do anytrhing special to distiquish buttons
                        # so click in a area to make sure screenshot is unique
                        
                       
                        
                       # how to best get to email once on outlook page
                       # and copy text from email
                       #execute in console document.activeElement
                       # until on correct element?
                       #setActive() 
                       # to set back to default document.activeElement.blur();
                       #.focus()
                       
                       
                       #opened dev tools with f12
                       # click on console using a saved button
                       # click to write code into console
         
                        
                        # and then enter
                        # maybe go by location of tab area
                        # and use enter
                        ...
                        if pressed_button=="/":# will need to test this to see if its gett he forward slash
                              ...
                              return
                          # use button to switch to using hotkeys
                          # so buttons become reusable
                        
                       
                        #if pressed_button=="\\":# will need to test this to see if its gett he forward slash
                        #      print("tab until you produce specific template with box around it see if can shape the template exactly to box of tab, shape  template captured ")
                        #      x_y_position_tuble=pyautogui.position() 
                        #      self.keyboard_and_mouse_press_and_time_list.append(["move_cursor","move_cursor",time.time(),x_y_position_tuble])
                        #      return
                          # print the whole screen
                          # ctrl a
                          # the benefit of this approach is dont need to worry about screen dimensions
                          # if cursor does not land in the right position
...
navigator.clipboard.writeText(textToCopy)
  ...
  .catch((err) => {
    ...
    def add_or_create_pyautogui_function_exe_files(self,exe_and_main_file_path,code_file_name_no_path,multiprocessing=False):
        """ """
        import re
        import os
        import time
    ...
    def add_or_create_business_processes_pre_made_bash_function_file(self,exe_and_main_file_path,code_file_name_no_path,exe_file_name):
        """ """
        import re
        import os
        import time
...
def {code_file_name_no_path}():
    ...
    input()"""  
        with open(business_processes_pre_made_bash_function_file_name,"w") as f:
            f.write(file_strr)
    def run_pyautogui_creation_script_on_all_other_computers_create_script_locally_to_fix_pixels(self):
        """ """          
    def create_pyautogui_file_of_inputs_and_video(self,previous_active_window,previous_active_window_url):
        ...
        python_file_str=self.create_pyauto_gui_function_str(self.keyboard_and_mouse_press_and_time_list,previous_active_window_url)
        ...
        print(multiprocess_pyautogui_scripts_with_tab_logic)
        ...
        single_process_file_name=self.add_or_create_pyautogui_function_exe_files(self.dir_to_store_script,code_file_name_no_path_for_function_import,multiprocessing=False)
        multi_process_file_name=self.add_or_create_pyautogui_function_exe_files(self.dir_to_store_script,code_file_name_no_path_for_function_import,multiprocessing=True)
        ...
        self.run_pyautogui_creation_script_on_all_other_computers_create_script_locally_to_fix_pixels()
