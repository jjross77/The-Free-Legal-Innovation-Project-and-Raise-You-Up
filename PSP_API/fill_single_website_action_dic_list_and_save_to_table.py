...
class fill_single():
    ...
    def retreive_problem_data_from_web(self,q,problemmm):
         import re
         import requests
         import pickle
    ...
    def figure_out_all_things_you_can_use_ask_many_questions_to_generate_expert_action_environment(self,saved_text_from_website_and_link_list,key):
        """ask many more questions to send to duckduckgo here 
        figure_out_all_things_you_can_use_ask_many_questions_on_objects_in_problem_words_in_problem_and_qualtites_to_generate_expert_problem_environment"""
...
if __name__ == '__main__':
    ...
    hist = Queue()
    p6 = Process(target=fill.retreive_problem_data_from_web, args=(hist,problemmm))
    p6.start()
    
    problemm="what is the biography of " + link_item_title
    bio = Queue()
    p7 = Process(target=fill.retreive_problem_data_from_web, args=(bio,problemmm))
    p7.start()
    
    problemm="what are the interests of " + link_item_title
    projects_int = Queue()
    p8 = Process(target=fill.retreive_problem_data_from_web, args=(projects_int,problemmm))
    p8.start()
    ...
    bio=process_website_text_and_lemmaize(bio)
    hist=process_website_text_and_lemmaize(hist)
    projects_int=process_website_text_and_lemmaize(projects_int)
    ...
    for person_comp_info_dic_with_action in person_comp_info_dic_with_action_list:
        ...
        duckduck = Queue()
        p = Process(target=fill.retreive_problem_data_from_web, args=(duckduck,problemmm))
        p.start()
        ...
        problemm="what are the steps i would take to " + person_comp_info_dic_with_action["action"]
        sub_action = Queue()
        p1 = Process(target=fill.retreive_problem_data_from_web, args=(sub_action,problemmm))
        p1.start()
             
        #get_placement_of_other_people_places_and_things_related_to_action
        problemm="what are other things i should consider relating to " + person_comp_info_dic_with_action["action"]
        placement = Queue()
        p2 = Process(target=fill.retreive_problem_data_from_web, args=(placement,problemmm))
        p2.start()
        
        # tools
        problemm="what are the tools i would use to " + person_comp_info_dic_with_action["action"]
        tools = Queue()
        p9 = Process(target=fill.retreive_problem_data_from_web, args=(tools,problemmm))
        p9.start()
        ...
        locate = Queue()
        p3 = Process(target=fill.retreive_problem_data_from_web, args=(locate,problemmm))
        p3.start()
        
        problemm="what are the resources i would take to " + person_comp_info_dic_with_action["action"]
        resource = Queue()
        p4 = Process(target=fill.retreive_problem_data_from_web, args=(resource,problemmm))
        p4.start()
        

        problemm="what are the skills that I would take to " + person_comp_info_dic_with_action["action"]
        skil = Queue()
        p5 = Process(target=fill.retreive_problem_data_from_web, args=(skil,problemmm))
        p5.start()
        ...
        p5.join()
        p6.join() 
        # still working on these 4 and then im done
        
        ### duckduck
        saved_text_from_website_and_link_list=duckduck.get()
        person_comp_info_dic_with_action=fill.fill_in_dic_with_duck_duck_go(person_comp_info_dic_with_action,saved_text_from_website_and_link_list) #get effects by doing this
        ...
        ### sub action!
        saved_text_from_website_and_link_list=sub_action.get()                                        
        person_comp_info_dic_with_action=fill.get_sub_action_list_data_use_patent_data_and_other(person_comp_info_dic_with_action,saved_text_from_website_and_link_list)# THIS IS KEY
        print(person_comp_info_dic_with_action)
        print('sub action sub list')
        
        ### placement!
        saved_text_from_website_and_link_list=placement.get()                                        
        person_comp_info_dic_with_action=fill.get_placement_of_other_people_places_and_things_related_to_action(person_comp_info_dic_with_action,saved_text_from_website_and_link_list)
        print('think mutliple moves ahead')
        print(person_comp_info_dic_with_action)
        
        ### tools!
        print('placement of other people')
        saved_text_from_website_and_link_list=tools.get()                                        
        person_comp_info_dic_with_action=fill.identify_tools_to_solve_the_problem(person_comp_info_dic_with_action,saved_text_from_website_and_link_list)
        ...
        person_comp_info_dic_with_action["work_experience"].insert(bio)
        person_comp_info_dic_with_action["work_experience"].insert(hist)
        person_comp_info_dic_with_action["projects_interested_in"].insert(projects_int)
        store_value_in_sql_table(person_comp_info_dic_with_action,table_name)
