...
class search_functions_gchild(search_functions_child):
    ...
    def get_intital_action_info_from_page_3(self,sel_soup,person_comp_info_dic_with_action_list,intital_link):
        ...
        found_noun=False
        entity_text_dic={}
        # called action action 2 and dont remove the remainder of the sentence
        unwanted_pronoun_list=["who", "whom", "what", "which","this", "there"]# there
        # need to not load up pages like history of a people
        # or find a  use for it, rewriting it or something to make it useful
        # how to get rde of bad links text
        # add  to the list these if results aren't good
        ...
        for i, sentence in enumerate(sentences_list):
            ...
            remove=self.none_sentence_remover(sentence)
            if remove==True:
                print('removed')
                continue           
            ...
            for i2,token in enumerate(sentence):
                ...
                if i2==len(sentence)-1:              
                  if proper_noun_or_pronoun_found==True and verb_found==True:
                      ...
                      proper_noun_count=sentence_word_list.count("PROPN")
                      if proper_noun_count>102:
                          print('skipping this one too many proper nouns')
                          print(f"proper_noun_count {proper_noun_count}")
                          #input(sentence_word_list)
                          #print(f"transformations {transformations}")
                          #print(f" noun_chunk {noun_chunk}")
                          person_comp_info_dic_with_action_copy=copy.deepcopy(person_comp_info_dic_with_action)
                          proper_noun_or_pronoun_found=False
                      ...
                      #sentence_word_list2=sentence_word_list[:50]
                      sentence_word_list2=" ".join(sentence_word_list)
                      sentence_word_list2 = re.sub(r'[\n\r\t]'," ",sentence_word_list2)
                      person_comp_info_dic_with_action_copy['full_action_sentence']=sentence_word_list2                     
                ...
                if poss=="PRON" or poss=="PROPN":
                        ...
                        #print(sentence)
                        #print(textt)
                        #print("pronoun fundddd")
                        #input('meowww')      
                    if textt in unwanted_pronoun_list:                  
                       #input(f"evil pron {textt}")
                       poss=str(poss)
                       poss=poss.lower()
...
#print(self.automated_problem_galaxy_dic)
    ...
    def generate_dag_from_current_position_to_figure_out_best_actions_you_can_take_for_different_objectives(self,user_positional_info_dic,all_guide_life_action_list_with_effects_dic,objective=""):
        ...
        column_list=["action_iterator","life_action","objective","weighted_avg_score","weights",            
        ...
        for iterr, life_action in enumerate(all_guide_life_action_list_with_effects_dic['action']):                 
                ...
                current_action_iterator=all_guide_life_action_list_with_effects_dic['action_iterator'][iterr]
                ...
                print('need to ensure no duplicates leave this for testing off ')
                ...
                for obejctive_doc in prompt_objectives_to_score_list:
                   ...
                   #print('we are limited by qualaites of input data so only get use these in choosing functions to sort actions so probably best way to incporrate game theory is considering other people involved in action/ environment of action')
                    ...
                    current_life_action_list=person_organizaion_action_list_dic[linkkkkk]
                    other_people_postiion_score,other_people_available_action_score=self.consider_other_peoples_and_orgnaizaions_positionality_and_there_available_actions_to_best_pick_your_action(str(current_life_action_list), user_position_context,person_organizaion_position_list_dic,person_organizaion_action_list_dic)                    
                    weights = [14.424, 14.421, 14.417,11.324,11.324,11.324,11.324,7,10.324,10.324,10.324]
                    # make the distance of things score negative
                    # money and time most valuable factors
                    amount = [guide_person_life_action_list_score, 
                              action_with_respect_to_objective_score,
                    ...
                    table_name="saved_action_scores"
                    dictionary_of_values={"action_iterator":current_action_iterator,
                                          "life_action":life_action,"objective":obejctive_doc,"weighted_avg_score":weighted_avg_score,"weights":weights,            
    ...
    def generate_NOO_life_dag_with_objective_most_financil_gains_best_investment_finder_money_personal_time_financial_forecasting(self,positional_info_dic,objective="make the most money"):
...
figure out all assets owned by business and figure out how to obtain this information from prospectus and add them to other businesses to generate value """
    ...
    #print("Connection closed.")
     return  sql_data_list_list
    def sort_and_upload_life_dag(self,all_user_life_action_dic):
        """ """
        #input(all_user_life_action_dic["life_action"])
        #input(all_user_life_action_dic["weighted_avg_score"])
        #input(all_user_life_action_dic["objective"])
        import copy
        temp_sorting_dic={}
        weighted_avg_score_list_copy=[]
        weighted_avg_score_list=[]
        saved_iterator=0
        action_list=[]
        objective_list=[]
        lower_then_current_score_list=[]
        weighted_avg_score_list_2=[]
        #weighted_avg_score_list=copy.deepcopy(all_user_life_action_dic["weighted_avg_score"])
        #weighted_avg_score_list=copy.deepcopy(all_user_life_action_dic["weighted_avg_score"])
        for i0, floatt in enumerate(all_user_life_action_dic["weighted_avg_score"]):    
            weighted_avg_score_list_2.append(float(floatt))
            all_user_life_action_dic["weighted_avg_score"][i0]=float(floatt)
            #print(all_user_life_action_dic["weighted_avg_score"])
            #input()            
        sorted_dic_all_user_life_action_dic={"weighted_avg_score":[],
                                          "life_action":[],
                                          "objective":[]}
        
        for i in range(len(weighted_avg_score_list_2)):
            max_value=max(weighted_avg_score_list_2) 
            #print(f"max_value {max_value}")
            #max_item_index=weighted_avg_score_list.index(max_value)         
            max_item_index=all_user_life_action_dic["weighted_avg_score"].index(max_value)
            sorted_dic_all_user_life_action_dic["life_action"].append(all_user_life_action_dic["life_action"][max_item_index])
            sorted_dic_all_user_life_action_dic["objective"].append(all_user_life_action_dic["objective"][max_item_index])
            sorted_dic_all_user_life_action_dic["weighted_avg_score"].append(all_user_life_action_dic["weighted_avg_score"][max_item_index])
            #print(f"c {weighted_avg_score_list_2}")
            temp_max_item_index=weighted_avg_score_list_2.index(max_value)
            del weighted_avg_score_list_2[temp_max_item_index]
            #print(f"s {weighted_avg_score_list_2}")
        #print(f" d{sorted_dic_all_user_life_action_dic['weighted_avg_score']}")
        #input(sorted_dic_all_user_life_action_dic["life_action"])
        #input(sorted_dic_all_user_life_action_dic["life_action"])
        
        # divde the dic by objective
        sorted_by_objective_dic={"helping the most people":{"life_action":[],"objective":[],"weighted_avg_score":[], },
                                 "making the most money":{"life_action":[],"objective":[], "weighted_avg_score":[],},
                                 "producing activism":{"life_action":[],"objective":[], "weighted_avg_score":[], },
                                 }
        #producing activism activism consists
        #making money 6 figure income teens way outlier 
        #helping people helping others important part life
        #print(f"sorted_dic_all_user_life_action_dic {sorted_dic_all_user_life_action_dic}")
        #input("meoww")
        for i4, objective in enumerate(sorted_dic_all_user_life_action_dic["objective"]):
            #input('objectivee')
            
            if "help" in objective:
                print('help found')
                sorted_by_objective_dic["helping the most people"]["life_action"].append(sorted_dic_all_user_life_action_dic["life_action"][i4])
                sorted_by_objective_dic["helping the most people"]["objective"].append(sorted_dic_all_user_life_action_dic["objective"][i4])
                sorted_by_objective_dic["helping the most people"]["weighted_avg_score"].append(sorted_dic_all_user_life_action_dic["weighted_avg_score"][i4])
                continue
                
            if "money" in objective:
                print('money found')
                sorted_by_objective_dic["making the most money"]["life_action"].append(sorted_dic_all_user_life_action_dic["life_action"][i4])
                sorted_by_objective_dic["making the most money"]["objective"].append(sorted_dic_all_user_life_action_dic["objective"][i4])
                sorted_by_objective_dic["making the most money"]["weighted_avg_score"].append(sorted_dic_all_user_life_action_dic["weighted_avg_score"][i4])
                continue
    
            if  "activism" in objective:
                print('actviism  found')

                sorted_by_objective_dic["producing activism"]["life_action"].append(sorted_dic_all_user_life_action_dic["life_action"][i4])
                sorted_by_objective_dic["producing activism"]["objective"].append(sorted_dic_all_user_life_action_dic["objective"][i4])
                sorted_by_objective_dic["producing activism"]["weighted_avg_score"].append(sorted_dic_all_user_life_action_dic["weighted_avg_score"][i4])
                continue
        return sorted_dic_all_user_life_action_dic,sorted_by_objective_dic
        ...
        return sorted_life_dags_dic_list
           ...
           # get the position that the score shuld be placed

        #sorted_life_action_tuple_for_objective=sorted(all_user_life_action_dic.items(),reverse=True)           
        #sorted_life_action_tuple_for_objective=sorted_life_action_tuple_for_objective[:20]# show top 10 hits
        #print(sorted_life_action_tuple_for_objective)
        
        
    def none_sentence_remover(self,sentence):       
        """remove sentences we dont want """  
        #print(f"lenthg {len(sentence)}")
        #input(sentence)
        remove=False
        # check data for patterns psql and then use this to 
        if len(sentence)<9:
            remove=True
        if "  " in sentence:
            remove=True
        ...
        #no newline charater sentences
        #spaces remove ones with 4 or more spaces
        return remove
    ...
    def search_for_dag_from_current_positon_3(self,prompt_objective):
        """ just reuse concept from glossary to make this faster
        stop words gone
        lemmaize text
        do a re findall
        to get scores
        check psp for what i did there for listbox
use objectives as categories to break up lists so dont run over every word    
so if it is a certain type of search fall into categoriy then run the list
check for certain value in list 

# going to make version 2 really fast
# over categorize it and search by that list
# so search through top 100 based on search in a category
# rank them based on sentence score 
so grab subset then add a last search based on a query and rerank those values
sort here
sorted_by_objective_dic={"helping the most people":{"life_action":[],"objective":[],"weighted_avg_score":[], },
                         "making the most money":{"life_action":[],"objective":[], "weighted_avg_score":[],},
                         "producing activism":{"life_action":[],"objective":[], "weighted_avg_score":[], }, }
"""
        ...
        problemmm=""
        # remove all stop words for better results
        self.conn = psycopg2.connect(dbname="can_law_accessible", user="postgres", password="Meganiscute")
        self.cur = self.conn.cursor()
        table_name ="saved_action_scores"
        column_list=["action_iterator","life_action","objective","weighted_avg_score","weights",            
        ...
        self.all_guide_life_action_list_with_scores_dic=self.retrieve_data_from_local_database(table_name,"",column_list,testing=False)
        all_guide_life_action_list_with_scores_dic=self.process_sql_data_for_searches(self.all_guide_life_action_list_with_scores_dic,column_list) 
        sorted_dic_all_user_life_action_dic2,sorted_by_objective_dic=self.sort_and_upload_life_dag(all_guide_life_action_list_with_scores_dic)
        ### grab the top 50 for each objective
        #print(sorted_by_objective_dic["helping the most people"]["life_action"][:50])
        #print('moooo')
        #input()
        help_words=["help"]
        money_words=["money"]
        activism_words=["activism"]
        prompt_found=False

        if "help" in prompt_objective and prompt_found==False:
            prompt_objective=prompt_objectives_to_score_list[0]
            sorted_objective_2=sorted_by_objective_dic["helping the most people"]["life_action"][:50]
            short_objective="helping the most people"
            prompt_found=True
            #print('found help')
            #print(sorted_objective_2)
            #input('WOO')
        if "money" in prompt_objective and prompt_found==False:
            prompt_objective=prompt_objectives_to_score_list[1]
            sorted_objective_2=sorted_by_objective_dic["making the most money"]["life_action"][:50]
            short_objective="making the most money"
            prompt_found=True


            
        if "activism" in prompt_objective and prompt_found==False:
            prompt_objective=prompt_objectives_to_score_list[2]
            sorted_objective_2=sorted_by_objective_dic["producing activism"]["life_action"][:50] 
            short_objective="producing activism"
            prompt_found=True
        ...
        prompt_objective=self.remove_stopwords(prompt_objective)

        #print(f"help_top50 {help_top50}")
        #input()
        
        for iterr, life_action in enumerate(sorted_objective_2):
            life_action=self.remove_stopwords(life_action)
            
            life_action_object_score=self.create_document_similarity_score(life_action,prompt_objective,typee="new") 
            objective=sorted_by_objective_dic[short_objective]["objective"][iterr]
            og_weighted_avg_score=sorted_by_objective_dic[short_objective]["weighted_avg_score"][iterr]
            weights = [1,1]
            # make the distance of things score negative
            # money and time most valuable factors
            amount = [float(life_action_object_score), 
                      float(og_weighted_avg_score),]
            #print(amount)s            
            user_weighted_avg_score = np.average(amount, weights=weights,axis=0)
            dictionary_of_values={"action_iterator":iterr,
                              "life_action":life_action,"objective":prompt_objective[:60],
                             "weighted_avg_score":user_weighted_avg_score,"weights":weights }
            dic_of_val_list.append(dictionary_of_values)
            # change this so it can go diretly be sorted after this
            sorted_dic_all_user_life_action_dic["weighted_avg_score"].append(user_weighted_avg_score)
            sorted_dic_all_user_life_action_dic["life_action"].append(life_action)
            sorted_dic_all_user_life_action_dic["objective"].append(prompt_objective[:60])
        #sort here as well
        self.conn = psycopg2.connect(dbname="can_law_accessible", user="postgres", password="Meganiscute")
        self.cur = self.conn.cursor()
        table_name="saved_action_scores_users" 
        for dictiary_value in dic_of_val_list:  
          self.store_value_in_sql_table(dictiary_value,table_name)
          
        #print(sorted_dic_all_user_life_action_dic["life_action"])
        #print(sorted_dic_all_user_life_action_dic["objective"])

        #input('not guilty')
          
        other_final_sorted_dic_all_user_life_action_dic,sort_and_upload_life_dag=self.sort_and_upload_life_dag(sorted_dic_all_user_life_action_dic)
        #print(sort_and_upload_life_dag)
        #input(' guilty')

        #print('we have reached the end')
        #input(final_sorted_dic_all_user_life_action_dic)
        return sort_and_upload_life_dag 
        #table_name="saved_action_scores_users" 
        #column_list=["action_iterator","life_action","objective","weighted_avg_score","weights", ]
        #self.user_life_action_list_with_scores_dic=self.retrieve_data_from_local_database(table_name,"",column_list,testing=False)
        #user_life_action_list_with_scores_dic=self.process_sql_data_for_searches(self.user_life_action_list_with_scores_dic,column_list)
        # doing this to make upload quicker
    
          #tim1=time.time()


        #for iterr, life_action in enumerate(all_guide_life_action_list_with_effects_dic['life_action']): 
            #objective =all_guide_life_action_list_with_effects_dic['objective ']
        #    weighted_avg_score=all_guide_life_action_list_with_effects_dic["weighted_avg_score"]
            #sort first here
            # then take the top 100 
   
        #final_all_sorted_life_action_for_objective_dic_list=[
        #    ]   
        #prompt_objectives_to_score_list[0]=self.remove_stopwords(prompt_objectives_to_score_list[0])
        #prompt_objectives_to_score_list[1]=self.remove_stopwords(prompt_objectives_to_score_list[1])
        #prompt_objectives_to_score_list[2]=self.remove_stopwords(prompt_objectives_to_score_list[2])
        #print(f"objective: {prompt_objectives_to_score_list[0]} ")
        #print(f"objective: {prompt_objectives_to_score_list[1]} ")
        #print(f"objective: {prompt_objectives_to_score_list[2]} ")
        #input('nelow')
        sorted_life_action_dic_for_objective={}
        dic_of_val_list=[]
        # need to prep this and add another function
        #otherwise it is too long
        
        for iterr, life_action in enumerate(all_guide_life_action_list_with_effects_dic['full_action_sentence']):                 
                #print(' check if used life action THIS IS WHAT I AM WORKING ON NOW')
                ...
                #print('need to remove cleaning below at somepoint fix upstream to remove commas')
                current_action_iterator=all_guide_life_action_list_with_effects_dic['action_iterator'][iterr]
                #life_action2=self.remove_stopwords(life_action)
                life_action2=life_action[:100]
                for obejctive_doc in prompt_objectives_to_score_list:    
                    life_action_object_score=self.create_document_similarity_score(life_action2,obejctive_doc,typee="new") 
                    dictionary_of_values={"action_iterator":current_action_iterator,
                                      "life_action":life_action,"objective":obejctive_doc[:60],
                                     "weighted_avg_score":life_action_object_score,}
                    dic_of_val_list.append(dictionary_of_values)
                    
        #table_name="saved_action_scores_user"
        #dictionary_of_values={"action_iterator":current_action_iterator,
        #                  "life_action":life_action,"objective":obejctive_doc[:60],
         #                 "weighted_avg_score":life_action_object_score,}           
        #for dictiary_value in dic_of_val_list:  
        ...
        #  self.store_value_in_sql_table(dictiary_value,table_name)
          ...
          #print(f" store{tim2-tim1}")
        #input()
                    #tim2=time.time()
                    #print(f"create doc{tim2-tim1}")
                    #input()                 
                          # this the column as key 

        
        
    def search_for_dag_from_current_positon_2(self, all_guide_life_action_list_with_effects_dic,all_guide_life_action_list_with_scores_dic,user_positional_info_dic):
        """ ALTERNATIVE NEURAL NETWORK combine gen dag and search into one search simplify it and rank"""
        import re
        import psycopg2
        import numpy as np
        sorted_life_dags_dic_list=[]
        ...
        for iterr, life_action in enumerate(all_guide_life_action_list_with_effects_dic['full_action_sentence']):                 
                ...
                column_value=column_value.replace(","," ")
                column_value=column_value.replace("'","")
                life_action2=re.sub("\"","",column_value)
                
                print(' need to remove action sentence stop words')
                print('need to ensure no duplicates remove value that is preventing repeats in certain actions')
                # neeed to think on this to remove duplicates
                life_action2=self.remove_stopwords(life_action2)
                life_action2=life_action2[:150]
                print(f"life_action {life_action2}")
                if life_action2 in all_guide_life_action_list_with_scores_dic["life_action"] :
                    print('life action!!!!!!!!!')
                    continue
                    #input()
                    #print(all_guide_life_action_list_with_scores_dic["life_action"])
                    #input('meow')
                    # how to control for sentene length and still get good results               
                for obejctive_doc in prompt_objectives_to_score_list:
                    ### processing amd getting values
                    life_sub_action_lists=all_guide_life_action_list_with_effects_dic["sub_steps_to_complete_actions"][iterr]
                    linkkkkk=all_guide_life_action_list_with_effects_dic["link"][iterr]
                    work_experience=all_guide_life_action_list_with_effects_dic["work_experience"][iterr]# use this to rank life_action_list in respect of objective give it a score
                    skills_required_to_perform_action=all_guide_life_action_list_with_effects_dic["skills_required_to_perform_action"][iterr]# use this to rank life_action_list in respect of objective give it a score
                    resources_required_to_perform_action=all_guide_life_action_list_with_effects_dic["resources_required_to_perform_action"][iterr]# use this to rank life_action_list in respect of objective give it a score

                    projects_interested_in=all_guide_life_action_list_with_effects_dic["projects_interested_in"][iterr]# use this to rank life_action_list in respect of objective give it a score
                    effects=all_guide_life_action_list_with_effects_dic["other_things_effected"][iterr]# use this to rank life_action_list in respect of objective give it a score
                    life_action_context=life_action2+ " "+life_sub_action_lists+" "+effects  +resources_required_to_perform_action  +skills_required_to_perform_action           
                    #print('STILL NEED TO WRITE THIS ONE use random qualtities we have')
                    #print('there is no info in user positon right now')
                    user_positional_info_context=str(user_positional_info_dic["property"])# use this to rank life_action_list in respect of objective give it a score
                    user_positional_info_context=user_positional_info_context+str(user_positional_info_dic["skills"])
                    user_positional_info_context=user_positional_info_context+str(user_positional_info_dic["connections"])
                    #print('use lemma and make it so good for exaxct word matches works need to write these algorhim and gahter the data ')               
                    ...
                    guide_person_life_action_list_score=self.create_document_similarity_score(guide_person_personal_info_context,obejctive_doc,typee="get_guides_life_action_list_score_for_objective")
                    action_with_respect_to_objective_score=self.create_document_similarity_score(life_action_context,obejctive_doc,typee="get_action_list_score_for_objective")
                    other_qualities_score=self.create_document_similarity_score(skills_required_to_perform_action,obejctive_doc,typee="use_all_qualtities_of_input_data_strategies_and_personal_info_in_different_ways_to_help_rank_action")
                    
                    user_personal_info_score=self.create_document_similarity_score(user_positional_info_context,guide_person_personal_info_context,typee="get_user_perosnal_info_score_for_objective")
                    only_action_words_with_objective=self.create_document_similarity_score(life_action2,obejctive_doc,typee="get_user_perosnal_info_score_for_objective")
                    ...
                    amount = [float(user_personal_info_score), 
                              ...
                              float(other_qualities_score),
                              float(guide_person_life_action_list_score),
                              float(only_action_words_with_objective)
                               ]
                    #print(amount)
                    ...
                    # need to create sub lists
                    print('need to create proper life_sub_action_lists to get this to work')
                    sorted_life_action_dic_for_objective[weighted_avg_score]=[life_action2,life_sub_action_lists,effects,obejctive_doc]  
                    #print(f"sorted_life_action_dic_for_objective value {sorted_life_action_dic_for_objective[weighted_avg_score]}")
                    # will need to write the below functions later
                    print(f"  weighted_avg_score   {weighted_avg_score}")
                    ...
                    table_name="saved_action_scores"
                    dictionary_of_values={"action_iterator":current_action_iterator,
                                          "life_action":life_action2,"objective":obejctive_doc[:60],
                                          "full_action_sentence":life_action,
                        ...
                        #"user_compared_to_guide_score": user_compared_to_guide_score,
                              "action_with_respect_to_objective_score":action_with_respect_to_objective_score,
                              "resources_score":other_qualities_score,
                              }
                              # this the column as key               
                    self.store_value_in_sql_table(dictionary_of_values,table_name)
                    
        
    def search_for_dag_from_current_positon(self, all_guide_life_action_list_with_effects_dic,all_guide_life_action_list_with_scores_dic,user_positional_info_dic,all_user_life_action_dic,objective="helping most people"):
        ...
        self.conn = psycopg2.connect(dbname="can_law_accessible", user="postgres", password="Meganiscute")
        self.cur = self.conn.cursor()
        #prompt_objectives_to_score_list=all_guide_life_action_list_with_scores_dic['objective']
        print('USE action scores compare against user scores how do i do this best')
        print(' need to generate user scores')
        #for iterr, life_action in enumerate(all_guide_life_action_list_with_effects_dic['action']):
        #    print(f"life_action {life_action}")
        for iterr,life_action in enumerate(all_guide_life_action_list_with_scores_dic['life_action']): 
                if life_action in all_user_life_action_dic['life_action']:
                    print('in already')
                    continue
                action_iterator=all_guide_life_action_list_with_scores_dic['action_iterator'][iterr]
                #print(f"success")
                #print('hi')
                #if objective ==prompt_objectives_to_score_list[0]:
                #print(f" OBJE {objective}")
                #input("meow")
                objective=all_guide_life_action_list_with_scores_dic['objective'][iterr]  


                #action_indext=all_guide_life_action_list_with_scores_dic['action_iterator'].index(iterr)
                
                #print(f"success")
                #print('hi')
                #print(f" OBJE {objective}"
                #print(len(all_guide_life_action_list_with_scores_dic['objective']))
                #if objective ==prompt_objectives_to_score_list[0]:
                #input("meow")

                guide_person_life_action_list_score=all_guide_life_action_list_with_scores_dic["guide_person_life_action_list_score"][iterr]
                action_with_respect_to_objective_score=all_guide_life_action_list_with_scores_dic["action_with_respect_to_objective_score"][iterr]                
                time_from_last_use_of_action_score=all_guide_life_action_list_with_scores_dic["time_from_last_use_of_action_score"][iterr],
                proxmity_of_action_to_other_past_used_action_score=all_guide_life_action_list_with_scores_dic["proxmity_of_action_to_other_past_used_action_score"][iterr]
                other_people_postiion_score=all_guide_life_action_list_with_scores_dic["other_people_postiion_score"][iterr]
                other_people_available_action_score=all_guide_life_action_list_with_scores_dic["other_people_available_action_score"][iterr]
                resources_score= all_guide_life_action_list_with_scores_dic["resources_score"][iterr]
                place_of_people_and_things_score= all_guide_life_action_list_with_scores_dic["place_of_people_and_things_score"][iterr]
                money_to_complete_action_score=all_guide_life_action_list_with_scores_dic["money_to_complete_action_score"][iterr]
                time_to_cmplete_action_score=all_guide_life_action_list_with_scores_dic["time_to_cmplete_action_score"][iterr]
                
                
                # use action key how to fit action key
                #if life_action in all_guide_life_action_list_with_effects_dic['action']:
                print('ADD A ITERATOR TO EACH TABLE TO MATCH ACTIONS to remove the error here')
                action_indext2=all_guide_life_action_list_with_effects_dic['action_iterator'].index(action_iterator)
                
                #action_indext2=all_guide_life_action_list_with_effects_dic['action'].index(action_iterator)
                #else:
                #    print('skip')
                    #skip need to make sure life actions are the same


                work_experience=all_guide_life_action_list_with_effects_dic["work_experience"][action_indext2]# use this to rank life_action_list in respect of objective give it a score
                projects_interested_in=all_guide_life_action_list_with_effects_dic["projects_interested_in"][action_indext2]# use this to rank life_action_list in respect of objective give it a score
                guide_person_personal_info_context=work_experience + " "+projects_interested_in
                
                user_positional_info_context=str(user_positional_info_dic["property"])# use this to rank life_action_list in respect of objective give it a score
                user_positional_info_context=user_positional_info_context+str(user_positional_info_dic["skills"])
                user_positional_info_context=user_positional_info_context+str(user_positional_info_dic["connections"])
                #print('prompt objective ')
                #input(objective)
                
                
                user_personal_info_score=self.create_document_similarity_score(user_positional_info_context,objective,typee="get_user_perosnal_info_score_for_objective")
                user_compared_to_guide_score=self.create_document_similarity_score(user_positional_info_context,guide_person_personal_info_context,typee="get_user_perosnal_info_score_for_guide_personal_info")   
                print(' the inputs are generaqted up top for this function consider_other_peoples_and_orgnaizaions_positionality_and_there_available_actions_to_best_pick_your_action')
                weights =[14.424,
                          14.421,
                ...
                #                                                          all_guide_life_action_list_with_effects_dic["other_things_effected"][iterr] ,
                 #                                                         objective]               
                table_name="saved_action_scores_users"
                dictionary_of_values={"action_iterator":action_iterator,"user_name":"Jasper","life_action":life_action,"objective":objective[:30],"weighted_avg_score":weighted_avg_score,"weights":weights,             
                          "user_personal_info_score": user_personal_info_score, 
...
class search_functions_g2_child(search_functions_gchild):
    ...
    def add_strategy(self,search,typee,user_positional_info_dic,objective="helping the most people"): # May include more arguments depending on URL parameters
       ...
       #print("and add a copy button to copy the document to clipboard")
        ...
        if new_table==True:
            table_name="saved_action_scores"
            dictionary_of_values={"action_iterator": "text","life_action":"text","full_action_sentence":'text',
                                  "objective":"text","weighted_avg_score":"text","weights":"text",            
        ...
        if create_table==True:
            table_name="saved_action_scores_users"
            dictionary_of_values={"action_iterator": "text","user_name":"text","life_action":"text","objective":"text","weighted_avg_score":"text","weights":"text",             
                      "user_personal_info_score": "text", 
        ...
        if generate_scores==True: 
            ...
            print('NOT USING THIS FOR NOW USE SEARCH INSTEAD')
        ...
        ### loading in final results to avoid duplicates
        #table_name ="saved_action_scores"
        #column_list=["action_iterator","life_action","objective","weighted_avg_score"]
        #all_user_life_action_dic=self.retrieve_data_from_local_database(table_name,"",column_list,testing=False)
        #all_user_life_action_dic=self.process_sql_data_for_searches(all_user_life_action_dic,column_list)     
        #input(f"all_user_life_action_dic {all_user_life_action_dic}")       
        # use user info here in search  STILL NEED TO WRITE THESE
        print('need to reduce the size of the documents  from being used as context')
        searched_life_dags_list_list=self.search_for_dag_from_current_positon_2(all_guide_life_action_list_dic,all_guide_life_action_list_with_scores_dic,user_positional_info_dic)
        print('transfer back to version 2 if this does not work the code liine above it seems to work now')
        prompt_objective="activism"
        # find a way to add extra context to prompt
        final_sorted_dic_all_user_life_action_dic=self.search_for_dag_from_current_positon_3(prompt_objective)
        
        print(f'FINAL {final_sorted_dic_all_user_life_action_dic}')
        input('woooof')
        # going to make version 2 really fast
        # over categorize it and search by that list
        # so search through top 100 based on search in a category
        #make this one below fast
        ...
        # remove duplicates
        # reloading in the results
        ### loading in final results to avoid duplicates need to test this
        #table_name ="saved_action_scores"
        #column_list=["life_action","objective","weighted_avg_score"]
        #all_user_life_action_dic=self.retrieve_data_from_local_database(table_name,"",column_list,testing=False)
        #all_user_life_action_dic=self.process_sql_data_for_searches(all_user_life_action_dic,column_list)         
        #sorted_dic_all_user_life_action_dic,sorted_by_objective_dic=self.sort_and_upload_life_dag(all_user_life_action_dic)
        #print(f"sorted_life_dags_dic_list {sorted_life_dags_dic_list}")
        #print(f"sorted_by_objective_dic,{sorted_by_objective_dic}")
        #print(f'sorted_life_dags_dic_list {sorted_life_dags_dic_list["life_action"]}')
        #print(f'sorted_life_dags_dic_list {sorted_life_dags_dic_list["life_action"][10]}')
