# -*- coding: utf-8 -*-
"""
Created on Thu Mar  2 13:08:54 2023
...
def Run_model():
    ...
    iiii= 0
    past_case_type= 0
    recording_index_value_for_current_sentence = 0
    ...
    for ii in range(2, storing):
        ...
        if os.path.exists(r"C:\Users\yyyyyyyyyyyyyyyyyyyy\Documents\sentence_list.pickle") == False:#1
            ...
            sentence=re.sub("/+", "", sentence) # preventing error with slashes in a string
    ...
    if os.path.exists(r"C:\Users\yyyyyyyyyyyyyyyyyyyy\Documents\existing_senteces_and_current_sentence_new.pickle") == True:# if_existing _sentences
            ...
            for iii, sentence, case_type in zip(range(len(sentence_list)), sentence_list, case_start_list_new):
                if iii==0:
                    past_case_type=1
                if iii>0:
                    past_case_type= case_type[iii-1]
                    
                    

                #the temp_sent checkers are used to check for whether the mathc is correct to ensure
                #we get the correct sentnece in current sentnece
                if past_case_type == case_type:
                    if iii>0:
                        
                        temp_sent1_checker = sentence_list[iii-1]
                        
                    
                    if iii> 1:    
                        temp_sent2_checker = sentence_list[iii-2]

                    

                    if sentence==current_sentence:                     # for when there is a match super important

                        if iii>0:
                            
                            
                            pre_sentence_1=re.search(f"{temp_sent1_checker}", existing_sentences)
                            
                            if pre_sentence_1 != None:
                                
                                
                                if iii>1:
                                    pre_sentence_2= re.search(f"{temp_sent2_checker}", existing_sentences)
                                    
                                    
                                    if pre_sentence_2 != None:
                                        #passed the second test woohoo
                                        print(iii)
                                        current_sentence_value_in_list=iii 
                                        break
                                # we got one match go here
                                    continue#fail2
                                print(iii) # this should be associated with if pre_sentence_1!= None
                                current_sentence_value_in_list=iii # passed the first test # but only is secodn value
                                break
                                
                            continue# for when it failed the first test
                        current_sentence_value_in_list=iii # for when no tests because it is first value in dataset
                    
                #still triyng to figure out this one    
    ...
    for sentence_2, case_numm, label2, label1 in zip(sentence_list_new, case_start_list_new, label_2_for_sentence_type_list_new, label_1_for_good_or_not_list_new):
        ...
        print(existing_sentences)
        recording_index_value_for_current_sentence+=1
        #print(sentence_2)
