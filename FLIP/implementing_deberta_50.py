# -*- coding: utf-8 -*-
"""
Created on Sat Feb 25 12:47:42 2023
...
def Run_model():
    ...
    if os.path.exists(r"D:\Kimlichcova\dicttt_of_sentence_embeddings.pickle") == True:
        with open(r"D:\Kimlichcova\dicttt_of_sentence_embeddings.pickle","rb") as ff:
    ...
    for i, sentence_2, case_numm in zip(range(1, len(sentence_list)),sentence_list, case_start_list):
        ...
        if i > num_of_embeddings:
            with open(r"D:\HPSCANS\existing_sentence_list.txt","a" , encoding="utf-8" ) as fff:
                fff.write(f"\n {existing_sentences} \n")
        ...
        if i > num_of_embeddings: # need to apply this else where
            ...
            try:
                ...
                if os.path.exists(r"D:\Kimlichcova\dicttt_of_sentence_embeddings.pickle") == True:
                    with open(r"D:\Kimlichcova\dicttt_of_sentence_embeddings.pickle","rb") as f:
                        original_case_dict=pickle.load(f)
                        for keys, values in original_case_dict.items():
                            dictt_of_sentence_embeddings[keys]= values
                    with open(r"D:\Kimlichcova\dicttt_of_sentence_embeddings.pickle","wb") as f:
