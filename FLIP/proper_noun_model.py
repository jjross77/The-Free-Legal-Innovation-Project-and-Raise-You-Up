# -*- coding: utf-8 -*-
"""
Created on Tue Jul 25 00:46:55 2023

@author: yyyyyyyyyyyyyyyyyyyy
"""
# use proper noun finder to ensure that this does not appear in the final list
import numpy as np
import pandas as pd
import torch
...
cur = conn.cursor()
cur.execute( f""" SELECT word, embedding, proper_noun_labels
                   FROM proper_noun_labels 
                   limit 50000
                    ;""")
...
for value in cur_result:
    ...
    if value[2]=="NO":
        label= 1
        list_of_labels_2_train.append(int(label))
...
model = network(768, 1000, 2).to("cuda") # hiddden layer should be 2 rd the size of the input layer
...
train_loss = []
def trainer(model, criterion, optimizer, dataloader, epochs=500):
...
torch.save(model.state_dict(), r"C:\Users\yyyyyyyyyyyyyyyyyyyy\Documents\Kimlichcova\proper_noun_model.pth")
